...
class _EditPropertyScreenState extends State<EditPropertyScreen> {
  ...
  String? _mapCommercialTypeToArabic(String? type) {
    ...
    return map[type] ?? type; // إذا لم تكن معروفة، نعيدها كما هي (أو نعيد 'آخر')
  }


  String? _mapCommercialTypeToEnglish(String? type) {
    if (type == null) return null;
    const map = {
      'متجر': 'shop',
      'مكتب': 'office',
    ...
    return map[type] ?? type; // إذا لم تكن معروفة، نعيدها كما هي
  }
  ...
  late List<PropertyImage> _currentImages; // الصور الحالية (قابلة للتعديل)
  ...
  void initState() {
    super.initState();
    _initializeData();
    _currentImages = List.from(widget.property.images); // نسخة قابلة للتعديل
  }
  ...
  void _initializeData() {
    ...
    _selectedOwnership = OwnershipType.values.firstWhere(
      ...
      orElse: () => OwnershipType.systemic_tabo,
    );
    ...
    if (p.commercialDetails != null) {
      ...
      // تحويل القيمة الإنجليزية إلى العربية
      _commercialType = _mapCommercialTypeToArabic(d.propertyType);
  ...
  Map<String, dynamic> _buildCurrentDataMap() {
    ...
    switch (_selectedCategory) {
      ...
      case Category.commercial:
        // تحويل النوع إلى الإنجليزية قبل الإرسال
        final englishType = _mapCommercialTypeToEnglish(_commercialType) ?? 'office';
        data['commercial_details'] = {
          'property_type': englishType,
  ...
  Future<void> _deleteImage(PropertyImage image) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text('هل أنت متأكد من حذف هذه الصورة نهائياً؟'),
    ...
    try {
      // استدعاء خدمة حذف الصورة
      final success = await _propertyService.deleteImage(image.id);
      if (success) {
        setState(() {
          _currentImages.removeWhere((img) => img.id == image.id);
        });
        _showSnack('تم حذف الصورة بنجاح', Colors.green);
      } else {
        _showSnack('فشل حذف الصورة', Colors.red);
      }
    } catch (e) {
                ...
                Step(
                  ...
                  content: Form(
                    ...
                    child: BaseInfoFields(
                      ...
                      // منع تعديل الفئة (readOnlyCategory = true)
                        ...
                        if (_currentImages.isNotEmpty)
                          Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                const Text(
                                  'الصور الحالية (اضغط على ✕ لحذف الصورة):',
                                  style: TextStyle(
                                ...
                                Text(
                                  '${_currentImages.length} صورة موجودة',
