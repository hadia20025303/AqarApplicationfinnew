...
import '../../../services/nominatim_service.dart'; // ⬅️ إضافة الخدمة
...
class _AddPropertyScreenState extends State<AddPropertyScreen> {
  ...
  final NominatimService _nominatimService = NominatimService(); // ⬅️ إضافة الخدمة
  int _currentStep = 0;
  ...
  OwnershipType _selectedOwnership = OwnershipType.systemic_tabo;
  ...
  Future<void> _updateLocationFromCoords(latlng.LatLng? location) async {
    if (location == null) {
      setState(() {
    ...
    try {
      final details = await _nominatimService.getLocationDetails(
        location.latitude,
        location.longitude,
      );
      ...
      debugPrint('فشل عكس الإحداثيات: $e');
  ...
  void _resetForm() {
    ...
    setState(() {
      ...
      _selectedOwnership = OwnershipType.systemic_tabo;
      _hasGarden = false;
      _hasPool = false;
      _parkingSpaces = 0;
  ...
  void _injectCategoryDetails(Map<String, dynamic> data) {
    ...
    case Category.commercial:
      data['commercial_details'] = {
        'property_type': _commercialType ?? 'office', // الآن هو المفتاح مباشرة
    ...
    switch (step) {
      ...
      case 2:
        child = LocationImagesFields(
          ...
          onLocationChanged: _updateLocationFromCoords, // ⬅️ تمرير الدالة الجديدة
        );
        break;
      case 3:
